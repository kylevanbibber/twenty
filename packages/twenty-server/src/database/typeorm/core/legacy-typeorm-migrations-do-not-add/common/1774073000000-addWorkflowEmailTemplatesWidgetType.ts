...
export class AddWorkflowEmailTemplatesWidgetType1774073000000 implements MigrationInterface {
  name = 'AddWorkflowEmailTemplatesWidgetType1774073000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "core"."pageLayoutWidget_type_enum" AS ENUM('VIEW', 'IFRAME', 'FIELDS', 'GRAPH', 'TIMELINE', 'TASKS', 'NOTES', 'FILES', 'EMAILS', 'CALENDAR', 'RICH_TEXT', 'WORKFLOW', 'WORKFLOW_VERSION', 'WORKFLOW_RUN', 'FIELD', 'STANDALONE_RICH_TEXT', 'FIELD_RICH_TEXT', 'FRONT_COMPONENT', 'RECORD_TABLE', 'EMAIL_THREAD', 'WORKFLOW_EMAIL_TEMPLATES')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "core"."pageLayoutWidget_type_enum_old" AS ENUM('VIEW', 'IFRAME', 'FIELDS', 'GRAPH', 'TIMELINE', 'TASKS', 'NOTES', 'FILES', 'EMAILS', 'CALENDAR', 'RICH_TEXT', 'WORKFLOW', 'WORKFLOW_VERSION', 'WORKFLOW_RUN', 'FIELD', 'STANDALONE_RICH_TEXT', 'FIELD_RICH_TEXT', 'FRONT_COMPONENT', 'RECORD_TABLE', 'EMAIL_THREAD')`,
    );
    await queryRunner.query(
      `ALTER TABLE "core"."pageLayoutWidget" ALTER COLUMN "type" DROP DEFAULT`,
    );
